...
/**
 * 1. Required populations must be present in population list 2. No extraneous populations that are
 * not associated with the scoring can be present
 */
@Slf4j
public class GroupScoringPopulationValidator
  ...
  public boolean isValid(Group group, ConstraintValidatorContext context) {
    ...
    try {
      ...
      List<MeasurePopulationOption> measurePopulationOptions =
          ScoringPopulationDefinition.SCORING_POPULATION_MAP.get(scoring);
      // make sure populations are from allowed list for group scoring

      return measurePopulationOptions.stream()
              .allMatch(
                  option -> {
                    Stream<Population> pops =
                        populations.stream()
                            .filter(
                                population -> {
                                  return Objects.equals(
                                      option.getMeasurePopulation(), population.getName());
                                });
                    ...
                    if (option.isRequired()) {
                      return matchingPopulation != null
                          && StringUtils.hasText(matchingPopulation.getDefinition());
                    }
                    return true;
                  })
          && populations.stream()
