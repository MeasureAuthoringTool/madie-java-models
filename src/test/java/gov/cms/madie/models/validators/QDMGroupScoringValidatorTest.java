...
public class QDMGroupScoringValidatorTest {
  ...
  public void testValidatorReturnsFalseForGroupScoringNotMatchingMeasureScoring() {
		...
		Group group = Group.builder().id("testGroupId").scoring(MeasureScoring.CONTINUOUS_VARIABLE.toString()).populationBasis("true").build();
  ...
  public void testValidatorReturnsFalseForMultipleGroupsHavingDifferentScoring() {
		measure.setScoring(MeasureScoring.COHORT.toString());
		Group group1 = Group.builder().id("testGroupId").scoring(MeasureScoring.CONTINUOUS_VARIABLE.toString()).populationBasis("true").build();
		Group group2 = Group.builder().id("testGroupId").scoring(MeasureScoring.COHORT.toString()).populationBasis("true").build();
