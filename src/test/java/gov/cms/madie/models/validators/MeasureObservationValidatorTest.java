...
class MeasureObservationValidatorTest {
  ...
  void testValidatorReturnsTrueForCohortGroupWithEmptyObservations() {
    boolean output =
        validator.isValid(
            cohortGroup.toBuilder().measureObservations(List.of()).build(), validatorContext);
    assertThat(output, is(true));
  }
  ...
  void testValidatorReturnsFalseForContinuousVariableGroupWithNoObservations() {
    boolean output =
        validator.isValid(
            cvGroup.toBuilder().measureObservations(List.of()).build(), validatorContext);
    assertThat(output, is(false));
  }
  ...
  void testValidatorReturnsFalseForContinuousVariableGroupWithObservationNoAggregateMethod() {
    boolean output =
        validator.isValid(
            cvGroup.toBuilder()
                .measureObservations(
                    List.of(MeasureObservation.builder().definition("obsABC").build()))
                .build(),
            validatorContext);
    assertThat(output, is(false));
  }
  ...
  void testValidatorReturnsTrueForRatioGroupWithNoObservations() {
    boolean output =
        validator.isValid(
            ratioGroup.toBuilder().measureObservations(List.of()).build(), validatorContext);
    assertThat(output, is(true));
  }
